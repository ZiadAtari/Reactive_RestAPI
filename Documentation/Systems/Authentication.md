...
- **Responsibilities**:
    ...
    - **Token Issuance**: Consumes `auth.token.issue` to generate user-specific tokens.
...
- **Purpose**: Manages the life-cycle of JWT tokens for external API calls and user sessions.
- **Key Features**:
    - **Service Token Generation**: Signs tokens for service-to-service calls (cached, auto-refresh).
    - **User Token Generation**: Generates short-lived (15m), unique tokens for authenticated users with claims (`sub`, `role`).
